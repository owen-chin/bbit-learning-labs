...
def main(ticker: str, price: float, sector: str) -> None:
    ...
    routingKey = f"Stock.{ticker}.{sector}".strip()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Process ticker, price, & float"
    )
    parser.add_argument("-t", "--ticker", type=str, required=True, help="stock ticker")
    parser.add_argument("-p", "--price", type=str, required=True, help="stock price")
    parser.add_argument("-s", "--sector", type=str, required=True, help="stock's sector")
